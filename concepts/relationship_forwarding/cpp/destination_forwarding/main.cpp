...
int main() {
    auto stage = pxr::UsdStage::Open("../../../usda/destination_forwarding.usda");
    auto prim = stage->GetPrimAtPath(pxr::SdfPath {"/Forwarder"});

    auto relationship = stage->GetPrimAtPath(pxr::SdfPath {"/SomePrim"}).GetRelationship(pxr::TfToken {"another"});
